      ...
      <el-row>
        ...
        <el-form-item  v-if="type!='info'"  label="内容" prop="neirong">
          <el-input v-model="ruleForm.neirong" 
              placeholder="内容" clearable></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `guanfangpiyao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanfangpiyaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {
