    ...
    >
      <el-row>
        ...
        <el-form-item v-if="type!='info'"  label="项目" prop="xiangmu">
          <el-select v-model="ruleForm.xiangmu" placeholder="请选择项目">
            <el-option
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `liuchengxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.liuchengxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {
