  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
          ...
          <el-button
            v-if="isAuth('diyizhiyuandiaochabiao','新增')"
            type="primary"
            @click="addOrUpdateHandler()"
            round
          >新增</el-button>
          <el-button
            v-if="isAuth('diyizhiyuandiaochabiao','删除')"
          ...
          >删除</el-button>
                    <el-button
            v-if="isAuth('diyizhiyuandiaochabiao','报表')"
            type="info"
            @click="chartDialog()"
            round
          >统计报表</el-button>
      ...
      <div class="table-content">
        <el-table
            v-if="isAuth('diyizhiyuandiaochabiao','查看')"
            :data="dataList"
            border
            ...
            <el-table-column
              ...
              <el-table-column
                  v-if="isAuth('diyizhiyuandiaochabiao','审核')"
                  prop="sfsh"
                  header-align="center"
                  align="center"
                  sortable
                ...
                label="操作">
                <template slot-scope="scope">
                                <el-button v-if="isAuth('diyizhiyuandiaochabiao','查看')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                                <el-button v-if="isAuth('diyizhiyuandiaochabiao','修改')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                <el-button v-if="isAuth('diyizhiyuandiaochabiao','删除')" type="text" icon="el-icon-delete" size="small" @click="deleteHandler(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
                                                                                                                                                <el-pagination
...
export default {
  ...
  methods: {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "group/diyizhiyuandiaochabiao/keximingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
      ...
      let params = {
        ...
        sort: 'id',
            ...
            params['xuejihao'] = '%' + this.searchForm.xuejihao + '%'
                                                                                                                                            ...
                                                                                                                                            this.$http({
        url: "diyizhiyuandiaochabiao/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    shHandler(){
      this.$confirm(`确定操作?`, "提示", {
        ...
        this.$http({
          url: "diyizhiyuandiaochabiao/update",
          method: "post",
          data: this.shForm
        }).then(({ data }) => {
          if (data && data.code === 0) {
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "diyizhiyuandiaochabiao/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
