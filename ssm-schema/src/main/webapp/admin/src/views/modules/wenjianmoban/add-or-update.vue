          ...
          <el-form-item v-if="ruleForm.biaoti" label="标题" prop="biaoti">
              <el-input v-model="ruleForm.biaoti" 
                placeholder="标题" readonly></el-input>
                        ...
                        <el-col :span="24">  
        <el-form-item v-if="type!='info'" label="文件" prop="wenjian">
          <file-upload
          tip="点击上传文件"
          action="file/upload"
          :limit="1"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `wenjianmoban/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wenjianmobanCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {
